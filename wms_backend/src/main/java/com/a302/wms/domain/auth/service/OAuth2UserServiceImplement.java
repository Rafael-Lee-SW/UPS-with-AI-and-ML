...
public class OAuth2UserServiceImplement extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest request) throws OAuth2AuthenticationException {
        ...
        log.info("{} user found: {}", email, existingUserOptional.orElse(null));

        if (existingUserOptional.isPresent()) {
            User existingUser = existingUserOptional.get();

            // 로그인 유형이 현재 OAuth2 요청과 일치하는지 확인
            if (existingUser.getSocialLoginType().name().equalsIgnoreCase(oauthClientName)) {
                // 이미 등록된 사용자라면 로그인 처리
                log.info("{} user logged in: {}", oauthClientName, email);
                return oAuth2User;
            } else {
                // 중복된 이메일 처리
                throw new CommonException(ResponseEnum.DUPLICATE_EMAIL, null);
            }
        } else {
            // 새로운 사용자 생성 및 저장
